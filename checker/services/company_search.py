    ...
    if request.method == "GET":
        ...
        if query.startswith("CM_"):
            ...
            try:
                ...
                logger.warning(
                    f"Component Query Filter built. About to execute Component.objects.filter..."
                )
                # all_components = Component.objects.filter(component_query_filter).order_by(comp_django_sort_field)
                logger.warning(
                    f"Component.objects.filter executed. About to call .count()..."
                )
                # component_count = all_components.count()
                component_count = 0  # Mock empty results
                logger.warning(
                    f"Component query executed. Filter: SKIPPED. Found {component_count} components."
                )
